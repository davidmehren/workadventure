...
import {
  extractDataFromPrivateRoomId,
  extractRoomSlugPublicRoomId,
  isRoomAnonymous,
} from "./RoomIdentifier";
...
export class GameRoom {
  ...
  constructor(
    ...
    if (this.anonymous) {
      ...
      const {
        ...
        worldSlug,
        roomSlug,
      } = extractDataFromPrivateRoomId(this.roomId);
  ...
  public join(socket: UserSocket, joinRoomMessage: JoinRoomMessage): User {
    ...
    if (positionMessage === undefined || positionMessage === null) {
  ...
  public leave(user: User) {
    ...
    if (userObj === undefined) {
      console.warn(
        "User ",
        user.id,
        "does not belong to this game room! It should!"
      );
    }
  ...
  private updateUserGroup(user: User): void {
    ...
    if (user.group === undefined) {
      ...
      const closestItem:
        | User
        | Group
        | null = this.searchClosestAvailableUserOrGroup(user);
  ...
  private searchClosestAvailableUserOrGroup(user: User): User | Group | null {
    let minimumDistanceFound: number = Math.max(
      this.minDistance,
      this.groupRadius
    );
  ...
  public static computeDistance(user1: User, user2: User): number {
    ...
    return Math.sqrt(
      Math.pow(user2Position.x - user1Position.x, 2) +
        Math.pow(user2Position.y - user1Position.y, 2)
    );
  }

  public static computeDistanceBetweenPositions(
    position1: PositionInterface,
    position2: PositionInterface
  ): number {
    return Math.sqrt(
