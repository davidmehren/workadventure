...
import {
  lazyLoadPlayerCharacterTextures,
  loadCustomTexture,
...
} from "../Login/SelectCharacterScene";
...
export class GameScene extends ResizableScene implements CenterListener {
  ...
  private actionableItems: Map<number, ActionableItem> = new Map<
    number,
    ActionableItem
  >();
  ...
  constructor(
    ...
    this.connectionAnswerPromise = new Promise<RoomJoinedMessageInterface>(
      (resolve, reject): void => {
        this.connectionAnswerPromiseResolve = resolve as (
          value?:
            | RoomJoinedMessageInterface
            | PromiseLike<RoomJoinedMessageInterface>
            | undefined
        ) => void;
      }
    );
  ...
  private async onMapLoad(data: any): Promise<void> {
    ...
    this.mapFile.tilesets.forEach((tileset) => {
      if (
        typeof tileset.name === "undefined" ||
        typeof tileset.image === "undefined"
      ) {
  ...
  create(): void {
    ...
    const mapDirUrl = this.MapUrlFile.substr(
      0,
      this.MapUrlFile.lastIndexOf("/")
    );
    ...
    for (const layer of this.mapFile.layers) {
      if (layer.type === "tilelayer") {
        this.addLayer(
          this.Map.createLayer(layer.name, this.Terrains, 0, 0).setDepth(depth)
        );
    ...
    if (depth === -2) {
      throw new Error(
        'Your map MUST contain a layer of type "objectgroup" whose name is "floorLayer" that represents the layer characters are drawn at.'
      );
    }
    ...
    this.presentationModeSprite = new PresentationModeIcon(
      this,
      36,
      this.game.renderer.height - 2
    );
    this.presentationModeSprite.on(
      "pointerup",
      this.switchLayoutMode.bind(this)
    );
    this.chatModeSprite = new ChatModeIcon(
      this,
      70,
      this.game.renderer.height - 2
    );
    this.chatModeSprite.on("pointerup", this.switchLayoutMode.bind(this));
    this.openChatIcon = new OpenChatIcon(
      this,
      2,
      this.game.renderer.height - 2
    );
      ...
      .then((onConnect: OnConnectInterface) => {
        ...
        this.connection.onUserMoved((message: UserMovedMessage) => {
          if (message === undefined || message === null) {
            console.error("Received empty message for onUserMoved event.");
        ...
        this.connection.onGroupUpdatedOrCreated(
        ...
        this.connection.onServerDisconnected(() => {
          ...
          const game: Phaser.Scene = new GameScene(
            this.room,
            this.MapUrlFile,
            gameSceneKey
          );
        ...
        this.UserMessageManager.setReceiveBanListener(
          this.bannedUser.bind(this)
        );
        ...
        this.CurrentPlayer.on(
          hasMovedEventName,
          this.pushPlayerPosition.bind(this)
        );
        ...
        this.ConsoleGlobalMessageManager = new ConsoleGlobalMessageManager(
          this.connection,
  ...
  private initCirclesCanvas(): void {
    ...
    circleElement = Object.values(this.textures.list).find(
      (object: Texture) => object.key === "circleSprite-red"
    );
    if (circleElement) {
    ...
    this.circleTexture = this.textures.createCanvas(
      "circleSprite-white",
      96,
      96
    );
    ...
    this.circleRedTexture = this.textures.createCanvas(
      "circleSprite-red",
      96,
      96
    );
  ...
  private playAudio(
    url: string | number | boolean | undefined,
    loop = false
  ): void {
    if (url === undefined) {
  ...
  private safeParseJSONstring(
    ...
    try {
      ...
      console.warn(
        'Invalid JSON found in property "' +
          propertyName +
          '" of the map:' +
          jsonString,
        e
  ...
  private triggerOnMapLayerPropertyChange() {
    ...
    this.gameMap.onPropertyChange(
      "openWebsite",
      (newValue, oldValue, allProps) => {
        if (newValue === undefined) {
          layoutManager.removeActionButton(
            "openWebsite",
            this.userInputManager
          );
          ...
          const openWebsiteTriggerValue = allProps.get(
            TRIGGER_WEBSITE_PROPERTIES
          );
          if (
            openWebsiteTriggerValue &&
            openWebsiteTriggerValue === ON_ACTION_TRIGGER_BUTTON
          ) {
        ...
        if (newValue === undefined) {
          ...
          const openJitsiRoomFunction = () => {
            ...
            if (JITSI_PRIVATE_MODE) {
              const adminTag = allProps.get("jitsiRoomAdminTag") as
                | string
                | undefined;
          ...
          if (
            jitsiTriggerValue &&
            jitsiTriggerValue === ON_ACTION_TRIGGER_BUTTON
          ) {
  ...
  private onMapExit(exitKey: string) {
    ...
    if (!roomId)
      throw new Error("Could not find the room from its exit key: " + exitKey);
  ...
  private getProperty(
    ...
    const obj = properties.find(
      (property: ITiledMapLayerProperty) =>
  ...
  private startUser(layer: ITiledMapLayer): PositionInterface {
    ...
    if (typeof tiles === "string") {
      throw new Error(
        "The content of a JSON map must be filled as a JSON array, not as a string"
      );
    }
    ...
    if (possibleStartPositions.length === 0) {
      console.warn(
        'The start layer "' + layer.name + '" for this map is empty.'
      );
      return {
        x: 0,
        y: 0,
  ...
  update(time: number, delta: number): void {
    ...
    const updatedPlayersPositions = this.playersPositionInterpolator.getUpdatedPositions(
      time
    );
    updatedPlayersPositions.forEach(
      (moveEvent: HasMovedEvent, userId: number) => {
        const player: RemotePlayer | undefined = this.MapPlayersByKey.get(
          userId
        );
        if (player === undefined) {
  ...
  private doUpdatePlayerPosition(message: MessageUserMovedInterface): void {
    ...
    if (player === undefined) {
      ...
      console.error(
        'Cannot update position of player with ID "' +
          message.userId +
          '": player not found'
      );
      return;
  ...
  private doShareGroupPosition(
    ...
    const sprite = new Sprite(
      ...
      groupPositionMessage.groupSize === MAX_PER_GROUP
        ? "circleSprite-red"
        : "circleSprite-white"
    );
  ...
  private updateCameraOffset(): void {
    ...
    this.cameras.main.startFollow(
      ...
      1,
      1,
      xCenter - this.game.renderer.width / 2,
      yCenter - this.game.renderer.height / 2
    );
  }
  ...
  public startJitsi(roomName: string, jwt?: string): void {
    ...
    const jitsiConfig = this.safeParseJSONstring(
      allProps.get("jitsiConfig") as string | undefined,
      "jitsiConfig"
    );
  ...
  private bannedUser() {
    ...
    this.scene.start(ErrorSceneName, {
      ...
      message:
        "If you want more information, you can contact us: workadventure@thecodingmachine.com",
    });
  }
}
