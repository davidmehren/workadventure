...
class JWTTokenManager {
  ...
  public async getUserUuidFromToken(
    token: unknown,
    ipAddress?: string,
    room?: string
  ): Promise<string> {
    if (!token) {
      throw new Error(
        "An authentication error happened, a user tried to connect without a token."
      );
    }
    ...
    return new Promise<string>((resolve, reject) => {
      Jwt.verify(token, SECRET_KEY, {}, (err, tokenDecoded) => {
        ...
        if (err) {
          console.error(
            "An authentication error happened, invalid JsonWebToken.",
            err
          );
          reject(
            new Error(
        ...
        if (ADMIN_API_URL) {
          ...
          if (ipAddress && room) {
            promise = this.verifyBanUser(
              tokenInterface.userUuid,
              ipAddress,
              room
            );
          }
