import {
  CharacterLayer,
  ExSocketInterface,
} from "../Model/Websocket/ExSocketInterface"; //TODO fix import by "_Model/.."
    ...
    this.app.ws("/admin/rooms", {
      ...
      message: (ws, arrayBuffer, isBinary): void => {
        try {
          ...
          const message: {
            event: string;
            message: { type: string; message: unknown; userUuid: string };
          } = JSON.parse(
            new TextDecoder("utf-8").decode(new Uint8Array(arrayBuffer))
          );

          if (message.event === "user-message") {
            const messageToEmit = message.message as {
              message: string;
              type: string;
              userUuid: string;
            };
  ...
  ioConnection() {
    this.app.ws("/room", {
      ...
      upgrade: (res, req, context) => {
        ...
        (async () => {
          ...
          try {
            ...
            const websocketKey = req.getHeader("sec-websocket-key");
            const websocketProtocol = req.getHeader("sec-websocket-protocol");
            const websocketExtensions = req.getHeader(
            ...
            const userUuid = await jwtTokenManager.getUserUuidFromToken(
              token,
              IPAddress,
              roomId
            );
            ...
            const characterLayerObjs: CharacterLayer[] = SocketManager.mergeCharacterLayersAndCustomTextures(
              characterLayers,
              memberTextures
            );
      ...
      open: (ws) => {
        ...
        if (ADMIN_API_URL) {
          adminApi
            .fetchMemberDataByUuid(client.userUuid)
            .then((res: FetchMemberDataByUuidResponse) => {
              if (!res.messages) {
                return;
              }
              res.messages.forEach((c: unknown) => {
                ...
                sendUserMessage.type = messageToSend.type;
                sendUserMessage.message = messageToSend.message;
      ...
      message: (ws, arrayBuffer, isBinary): void => {
        ...
        const message = ClientToServerMessage.decode(
          new Uint8Array(arrayBuffer)
        );

        if (message.viewportMessage !== null) {
          socketManager.handleViewport(
            client,
            message.viewportMessage as ViewportMessage
          );
        } else if (
          ...
          message.setPlayerDetailsMessage !== undefined &&
          message.setPlayerDetailsMessage !== null
        ) {
          socketManager.handleSetPlayerDetails(
            client,
            message.setPlayerDetailsMessage as SetPlayerDetailsMessage
          );
        } else if (
          ...
          message.reportPlayerMessage !== undefined &&
          message.reportPlayerMessage !== null
        ) {
          socketManager.handleReportMessage(
            client,
            message.reportPlayerMessage as ReportPlayerMessage
          );
        } else if (
